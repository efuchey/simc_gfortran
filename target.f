...
c<<<<<<< HEAD
c	 else if (targ%can .eq.3) then          !2017 target 10 cm cells
c	    s_Al = s_Al + 0.013                 !avg. of 3 loops
c=======
...
20	continue
	...
	if (electron_arm.eq.1) then		!electron is in HMS
	  ...
	  forward_path = (targ%length/2.-zpos) / abs(cos(theta-targ%angle))
c<<<<<<< HEAD
c	else if (electron_arm.eq.5 .or. electron_arm.eq.6) then	!SHMS
c	  s_Al = (0.02+0.01)*inch_cm !20mil scattering chamber window + 10 mil entrance window
c	  s_air = 57.27
c 	  s_kevlar = 0.0
c	  s_mylar = 0.0
c=======
